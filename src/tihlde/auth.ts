...
export const getAuthToken = ({ req, res }: NextResponseRequest): string | undefined => {
  const token = getCookie(AUTH_TOKEN_COOKIE_KEY, { req, res });

  return typeof token === 'string' ? token : undefined;
};

export const getAuthHeaders = ({ req, res }: NextResponseRequest): Pick<AxiosRequestConfig<unknown>, 'headers'> => {
  const token = getAuthToken({ req, res });

  return { headers: { 'x-csrf-token': token } };
};

export const authenticate = async ({ user_id, password, req, res }: { user_id: string; password: string } & NextResponseRequest) => {
  try {
